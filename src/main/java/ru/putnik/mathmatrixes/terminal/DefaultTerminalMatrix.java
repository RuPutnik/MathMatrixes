...
public class DefaultTerminalMatrix {
    ...
    public static String size(DefaultTerminalMatrix defaultMatrix){
        if(defaultMatrix.completedMatrix==null|| defaultMatrix.completedMatrix.length==0){
            return "0x0";
        }else {
            return String.valueOf(defaultMatrix.completedMatrix.length) + "x" + String.valueOf(defaultMatrix.completedMatrix[0].length);
        }
    }
    public static double valueAt(DefaultTerminalMatrix defaultMatrix, int column, int row){
        return defaultMatrix.completedMatrix[row-1][column-1];
    }
    public static double trace(DefaultTerminalMatrix defaultMatrix){
        if(DefaultTerminalMatrix.isSquare(defaultMatrix)){
    ...
    public static DefaultTerminalMatrix set(DefaultTerminalMatrix matrix, int row, int column, double value){
    ...
    public static int getCountColumns(DefaultTerminalMatrix defaultMatrix){
        String sizeMatrix= DefaultTerminalMatrix.size(defaultMatrix);
    ...
    public static boolean equalsSizes(DefaultTerminalMatrix matrix1, DefaultTerminalMatrix matrix2) {
        return DefaultTerminalMatrix.size(matrix1).equals(DefaultTerminalMatrix.size(matrix2));
    }
    ...
    public static double det(DefaultTerminalMatrix matrix){
        double result=0;
        if(DefaultTerminalMatrix.isSquare(matrix)){
         ...
         // если да
            ...
            for(int row=0;row<(countRow-(sizeStageMatrix-1));row++){ // тут начинается перебор матриц q-го порядка
                for(int column=0;column<(countColumns-(sizeStageMatrix-1));column++){
                    ...
                    if(!(DefaultTerminalMatrix.det(new DefaultTerminalMatrix(stageMatrix))==0)){ // если определитель матрицы отличен от нуля
                     // то
    ...
    private static double cofactor(DefaultTerminalMatrix matrix, int rowNumber, int columnNumber){
        ...
        double detMatrix= DefaultTerminalMatrix.det(DefaultTerminalMatrix.moll1(matrix,rowNumber,columnNumber));
    ...
    public static DefaultTerminalMatrix reverse(DefaultTerminalMatrix matrix){
        if(DefaultTerminalMatrix.getCountRows(matrix)== DefaultTerminalMatrix.getCountColumns(matrix)){
            if(DefaultTerminalMatrix.det(matrix)!=0) {
                double[][] values = new double[matrix.completedMatrix.length][matrix.completedMatrix[0].length];

                for (int row=0;row<matrix.completedMatrix.length;row++){
                    for (int column=0;column<matrix.completedMatrix[0].length;column++){
                        values[row][column]= DefaultTerminalMatrix.cofactor(matrix,row,column);
                    }
                }

                DefaultTerminalMatrix unionMatrix = new DefaultTerminalMatrix(values);
                DefaultTerminalMatrix reverseMatrix = DefaultTerminalMatrix.division(DefaultTerminalMatrix.trans(unionMatrix), DefaultTerminalMatrix.det(matrix));
